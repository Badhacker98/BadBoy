...
from BadBoy.exceptions import DependencyMissingError
...
from ..version import BadBoy_version as ult_ver
...
def BadBoy_cmd(
    pattern=None, manager=False, BadBoy_bot=BadBoy_bot, asst=asst, **kwargs
):
    ...
    def decor(dec):
        async def wrapp(ult):
            ...
            try:
                ...
                await BadBoy_bot.disconnect()
                await asyncio.sleep(fwerr.seconds + 10)
                await BadBoy_bot.connect()
                await asst.send_message(
                    udB.get_key("LOG_CHANNEL"),
