...
def telegraph_client():
    ...
    gd_name = BadBoy_bot.full_name
    short_name = gd_name[:30]
    profile_url = (
        f"https://t.me/{BadBoy_bot.me.username}"
        if BadBoy_bot.me.username
        else "https://t.me/TeamBadBoy"
    )
    try:
        ...
        if "SHORT_NAME_TOO_LONG" in str(er):
            TelegraphClient.create_account(
                short_name="BadBoyuser", author_name=gd_name, author_url=profile_url
            )
        else:
            LOGS.exception(er)
            return
...
async def Carbon(
    ...
    **kwargs,
    ...
    if rayso:
        ...
        kwargs["title"] = kwargs.get("title", "BadBoy")
    else:
        kwargs["code"] = code
