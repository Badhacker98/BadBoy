...
if run_as_module:
    ...
    async def safeinstall(event):
        ...
        if plug in HELP:
            ...
            for i in HELP[plug]:
                output += i
            output += "\n© @TeamBadBoy"
            await eod(ok, f"✓ `BadBoy - Installed`: `{plug}` ✓\n\n{output}")
    ...
    async def heroku_logs(event):
        ...
        ok = app.get_log()
        with open("BadBoy-heroku.log", "w") as log:
            log.write(ok)
        await event.client.send_file(
            event.chat_id,
            file="BadBoy-heroku.log",
            thumb=ULTConfig.thumb,
            caption="**BadBoy Heroku Logs.**",
        )

        os.remove("BadBoy-heroku.log")
        await xx.delete()
    ...
    def gen_chlog(repo, diff):
        ...
        ch_log = tldr_log = ""
        ch = f"<b>BadBoy {BadBoy_version} updates for <a href={UPSTREAM_REPO_URL}/tree/{ac_br}>[{ac_br}]</a>:</b>"
        ch_tl = f"BadBoy {BadBoy_version} updates for {ac_br}:"
        d_form = "%d/%m/%y || %H:%M"
