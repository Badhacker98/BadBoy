...
async def autoupdate_local_database():
    from .. import Var, asst, udB, BadBoy_bot

    global db_url
    db_url = (
        udB.get_key("TGDB_URL") or Var.TGDB_URL or BadBoy_bot._cache.get("TGDB_URL")
    )
    if db_url:
        _split = db_url.split("/")
        _channel = _split[-2]
...
async def startup_stuff():
    ...
    if TZ and timezone:
        try:
            ...
            LOGS.critical(
                "Incorrect Timezone ,\nCheck Available Timezone From Here https://graph.org/BadBoy-06-18-2\nSo Time is Default UTC"
            )
            os.environ["TZ"] = "UTC"
            time.tzset()
...
async def autobot():
    from .. import udB, BadBoy_bot

    if udB.get_key("BOT_TOKEN"):
        return
    await BadBoy_bot.start()
    ...
    if who.username:
        ...
        username = "BadBoy_" + (str(who.id))[5:] + "_bot"
    bf = "@BotFather"
    await BadBoy_bot(UnblockRequest(bf))
    await BadBoy_bot.send_message(bf, "/cancel")
    await asyncio.sleep(1)
    await BadBoy_bot.send_message(bf, "/newbot")
    await asyncio.sleep(1)
    isdone = (await BadBoy_bot.get_messages(bf, limit=1))[0].text
    if isdone.startswith("That I cannot do.") or "20 bots" in isdone:
        ...
        sys.exit(1)
    await BadBoy_bot.send_message(bf, name)
    await asyncio.sleep(1)
    isdone = (await BadBoy_bot.get_messages(bf, limit=1))[0].text
    if not isdone.startswith("Good."):
        await BadBoy_bot.send_message(bf, "My Assistant Bot")
        await asyncio.sleep(1)
        isdone = (await BadBoy_bot.get_messages(bf, limit=1))[0].text
        if not isdone.startswith("Good."):
            LOGS.critical(
            ...
            sys.exit(1)
    await BadBoy_bot.send_message(bf, username)
    await asyncio.sleep(1)
    isdone = (await BadBoy_bot.get_messages(bf, limit=1))[0].text
    await BadBoy_bot.send_read_acknowledge("botfather")
    if isdone.startswith("Sorry,"):
        ran = randint(1, 100)
        username = "BadBoy_" + (str(who.id))[6:] + str(ran) + "_bot"
        await BadBoy_bot.send_message(bf, username)
        await asyncio.sleep(1)
        isdone = (await BadBoy_bot.get_messages(bf, limit=1))[0].text
    if isdone.startswith("Done!"):
        ...
        await enable_inline(BadBoy_bot, username)
        LOGS.info(
...
async def autopilot():
    ...
    if channel:
        try:
            chat = await BadBoy_bot.get_entity(channel)
    ...
    if not channel:
        ...
        async def _save(exc):
            udB._cache["LOG_CHANNEL"] = BadBoy_bot.me.id
            await asst.send_message(
                BadBoy_bot.me.id, f"Failed to Create Log Channel due to {exc}.."
            )
    ...
    assistant = True
    try:
        await BadBoy_bot.get_permissions(int(channel), asst.me.username)
    except UserNotParticipantError:
        try:
            await BadBoy_bot(InviteToChannelRequest(int(channel), [asst.me.username]))
        except BaseException as er:
    ...
    if isinstance(chat.photo, ChatPhotoEmpty):
        photo, _ = await download_file(name="https://telegra.ph/file/52608ea608757134bd201.jpg")
...
async def customize():
    ...
    try:
        ...
        if not BadBoy_bot.me.username:
            sir = BadBoy_bot.me.first_name
        else:
        ...
        await asyncio.sleep(1)
        await BadBoy_bot.send_message("botfather", "/cancel")
        await asyncio.sleep(1)
        await BadBoy_bot.send_message("botfather", "/setuserpic")
        await asyncio.sleep(1)
        isdone = (await BadBoy_bot.get_messages("botfather", limit=1))[0].text
        if isdone.startswith("Invalid bot"):
            LOGS.info("Error while trying to customise assistant, skipping...")
            return
        await BadBoy_bot.send_message("botfather", UL)
        await asyncio.sleep(1)
        await BadBoy_bot.send_file("botfather", file)
        await asyncio.sleep(2)
        await BadBoy_bot.send_message("botfather", "/setabouttext")
        await asyncio.sleep(1)
        await BadBoy_bot.send_message("botfather", UL)
        await asyncio.sleep(1)
        ...
        await asyncio.sleep(2)
        await BadBoy_bot.send_message("botfather", "/setdescription")
        await asyncio.sleep(1)
        await BadBoy_bot.send_message("botfather", UL)
        await asyncio.sleep(1)
        await BadBoy_bot.send_message(
...
async def plug(plugin_channels):
    ...
    for chat in plugin_channels:
        LOGS.info(f"{'•'*4} {chat}")
        try:
            async for x in BadBoy_bot.iter_messages(
                ...
                if not os.path.exists(plugin):
                    ...
                    try:
                        ...
                        LOGS.info(f"BadBoy - PLUGIN_CHANNEL - ERROR - {plugin}")
                        LOGS.exception(e)
                        os.remove(plugin)
        except Exception as er:
            LOGS.exception(er)
...
async def fetch_ann():
    ...
    try:
        updts = await async_searcher(
            "https://BadBoy-api.vercel.app/announcements", post=True, re_json=True
        )
        for upt in updts:
            key = list(upt.keys())[0]
            if key not in get_:
...
• Hᴇʀᴇ, ᴀʀᴇ ᴛʜᴇ Sᴏᴍᴇ Bᴀsɪᴄ sᴛᴜғғ ғʀᴏᴍ, ᴡʜᴇʀᴇ ʏᴏᴜ ᴄᴀɴ Kɴᴏᴡ, ᴀʙᴏᴜᴛ ɪᴛs Usᴀɢᴇ."""
        ...
        if prev_spam:
            try:
                await BadBoy_bot.delete_messages(chat_id, int(prev_spam))
            except Exception as E:
    ...
    try:
        ...
        try:
            await (await BadBoy_bot.send_message(chat_id, str(e))).delete()
            spam_sent = await asst.send_message(chat_id, MSG, file=PHOTO, buttons=BTTS)
        ...
        LOGS.info(el)
        try:
            spam_sent = await BadBoy_bot.send_message(chat_id, MSG)
...
async def WasItRestart(udb):
    ...
    if not key:
        return
    from .. import asst, BadBoy_bot

    try:
        data = key.split("_")
        who = asst if data[0] == "bot" else BadBoy_bot
...
async def enable_inline(BadBoy_bot, username):
    bf = "BotFather"
    await BadBoy_bot.send_message(bf, "/setinline")
    await asyncio.sleep(1)
    await BadBoy_bot.send_message(bf, f"@{username}")
    await asyncio.sleep(1)
    await BadBoy_bot.send_message(bf, "Search")
    await BadBoy_bot.send_read_acknowledge(bf)
