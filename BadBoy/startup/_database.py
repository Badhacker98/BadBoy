...
class MongoDB(_BaseDatabase):
    def __init__(self, key, dbname="BadBoyDB"):
        self.dB = MongoClient(key, serverSelectionTimeoutMS=5000)
        self.db = self.dB[dbname]
        super().__init__()

    def __repr__(self):
        return f"<BadBoy.MonGoDB\n -total_keys: {len(self.keys())}\n>"
    ...
    def flushall(self):
        self.dB.drop_database("BadBoyDB")
        self._cache.clear()
        return True
...
class SqlDB(_BaseDatabase):
    def __init__(self, url):
        ...
        try:
            ...
            self._cursor.execute(
                "CREATE TABLE IF NOT EXISTS BadBoy (BadBoyCli varchar(70))"
            )
    ...
    def usage(self):
        self._cursor.execute(
            "SELECT pg_size_pretty(pg_relation_size('BadBoy')) AS size"
        )
    ...
    def keys(self):
        self._cursor.execute(
            "SELECT column_name FROM information_schema.columns WHERE table_schema = 'public' AND table_name  = 'BadBoy'"
        )  # case sensitive
        data = self._cursor.fetchall()
    ...
    def get(self, variable):
        try:
            self._cursor.execute(f"SELECT {variable} FROM BadBoy")
    ...
    def set(self, key, value):
        try:
            self._cursor.execute(f"ALTER TABLE BadBoy DROP COLUMN IF EXISTS {key}")
        except (psycopg2.errors.UndefinedColumn, psycopg2.errors.SyntaxError):
            pass
        except BaseException as er:
            LOGS.exception(er)
        self._cache.update({key: value})
        self._cursor.execute(f"ALTER TABLE BadBoy ADD {key} TEXT")
        self._cursor.execute(f"INSERT INTO BadBoy ({key}) values (%s)", (str(value),))
        return True

    def delete(self, key):
        try:
            self._cursor.execute(f"ALTER TABLE BadBoy DROP COLUMN {key}")
        except psycopg2.errors.UndefinedColumn:
            return False
        return True
    ...
    def flushall(self):
        self._cache.clear()
        self._cursor.execute("DROP TABLE BadBoy")
        self._cursor.execute(
            "CREATE TABLE IF NOT EXISTS BadBoy (BadBoyCli varchar(70))"
        )
        return True
