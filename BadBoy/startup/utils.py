...
def load_addons(plugin_name):
    ...
    if base_name.startswith("__"):
        return
    from BadBoy import fns

    from .. import HNDLR, LOGS, asst, udB, BadBoy_bot
    from .._misc import _supporter as config
    from .._misc._assistant import asst_cmd, callback, in_pattern
    from .._misc._decorators import BadBoy_cmd
    ...
    for path in configPaths:
        modules[path] = config
    modules["BadBoy.functions"] = fns
    mod.LOG_CHANNEL = udB.get_key("LOG_CHANNEL")
    mod.udB = udB
    mod.asst = asst
    mod.tgbot = asst
    mod.BadBoy_bot = BadBoy_bot
    mod.ub = BadBoy_bot
    mod.bot = BadBoy_bot
    mod.BadBoy = BadBoy_bot
    mod.borg = BadBoy_bot
    mod.telebot = BadBoy_bot
    mod.jarvis = BadBoy_bot
    mod.friday = BadBoy_bot
    mod.eod = eod
    mod.edit_delete = eod
    mod.LOGS = LOGS
    ...
    mod.BadBoy_cmd = BadBoy_cmd
    mod.on_cmd = BadBoy_cmd
