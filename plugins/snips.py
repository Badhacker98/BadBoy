...
from BadBoy._misc import sudoers
from BadBoy.dB.snips_db import add_snip, get_snips, list_snip, rem_snip
...
async def an(e):
    ...
    BadBoy_bot.add_handler(add_snips, events.NewMessage())


@BadBoy_cmd(pattern="remsnip( (.*)|$)")
async def rs(e):
    wrd = (e.pattern_match.group(1).strip()).lower()
    if not wrd:
    ...
    for z in xx:
        if k := get_snips(z):
            ...
            if rep:
                ...
                await BadBoy_bot.send_message(e.chat_id, msg, file=media)
