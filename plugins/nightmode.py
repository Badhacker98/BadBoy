...
from . import get_string, udB, BadBoy_bot, BadBoy_cmd

keym = KeyManager("NIGHT_CHATS", cast=list)


@BadBoy_cmd(pattern="nmtime( (.*)|$)")
...
@BadBoy_cmd(pattern="addnm( (.*)|$)")
async def add_grp(e):
    if pat := e.pattern_match.group(1).strip():
        try:
            keym.add((await BadBoy_bot.get_entity(pat)).id)
...
@BadBoy_cmd(pattern="remnm( (.*)|$)")
async def r_em_grp(e):
    if pat := e.pattern_match.group(1).strip():
        try:
            keym.remove((await BadBoy_bot.get_entity(pat)).id)
...
@BadBoy_cmd(pattern="listnm$")
async def rem_grp(e):
    chats = keym.get()
    name = "NightMode Groups Are-:\n\n"
    for x in chats:
        try:
            ok = await BadBoy_bot.get_entity(x)
...
async def open_grp():
    for chat in keym.get():
        try:
            ...
            await BadBoy_bot.send_message(chat, "**NightMode Off**\n\nGroup Opened 🥳.")
