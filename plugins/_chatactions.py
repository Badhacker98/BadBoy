...
from BadBoy.dB import stickers
from BadBoy.dB.echo_db import check_echo
from BadBoy.dB.forcesub_db import get_forcesetting
from BadBoy.dB.gban_mute_db import is_gbanned
from BadBoy.dB.greetings_db import get_goodbye, get_welcome, must_thank
from BadBoy.dB.nsfw_db import is_profan
from BadBoy.fns.helper import inline_mention
from BadBoy.fns.tools import async_searcher, create_tl_btn, get_chatbot_reply

try:
    from ProfanityDetector import detector
except ImportError:
    detector = None
from . import LOG_CHANNEL, LOGS, asst, get_string, types, udB, BadBoy_bot
from ._inline import something


@BadBoy_bot.on(events.ChatAction())
...
async def DummyHandler(ult):
    ...
    if (
        ...
        if not user.bot:
            ...
            try:
                await BadBoy_bot(GetParticipantRequest(int(joinchat), user.id))
            except UserNotParticipantError:
                await BadBoy_bot.edit_permissions(
                    ult.chat_id, user.id, send_messages=False
                )
                res = await BadBoy_bot.inline_query(
    ...
    if ult.user_joined or ult.added_by:
        ...
        # gbans and @BadBoyBans checks
        if udB.get_key("BadBoy_BANS"):
            try:
                is_banned = await async_searcher(
                    "https://bans.BadBoy.tech/api/status",
                    json={"userId": user.id},
                    post=True,
                    re_json=True,
                )
                if is_banned["is_banned"]:
                    ...
                    await ult.respond(
                        f'**@BadBoyBans:** Banned user detected and banned!\n`{str(is_banned)}`.\nBan reason: {is_banned["reason"]}',
                    )
...
@BadBoy_bot.on(events.NewMessage(incoming=True))
...
@BadBoy_bot.on(events.Raw(types.UpdateUserName))
