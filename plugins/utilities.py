...
from BadBoy._misc._assistant import asst_cmd
from BadBoy.dB.gban_mute_db import is_gbanned
from BadBoy.fns.tools import get_chat_and_msgid
...
from BadBoy.fns.info import get_chat_info
...
@BadBoy_cmd(pattern="kickme$", fullsudo=True)
async def leave(ult):
...
@BadBoy_cmd(pattern="paste( (.*)|$)", manager=True, allow_all=True)
async def _(event):
    try:
...
async def telegraphcmd(event):
    xx = await event.eor(get_string("com_1"))
    match = event.pattern_match.group(1).strip() or "BadBoy"
    reply = await event.get_reply_message()
    if not reply:
...
@BadBoy_cmd(pattern="suggest( (.*)|$)", manager=True)
...
async def get_restriced_msg(event):
    ...
    if not (chat and msg):
        return await event.eor(
            f"{get_string('gms_1')}!\nEg: `https://t.me/PUNJABI_CHATTING_HUB or `https://t.me/PBX_PERMOT`"
        )
    try:
