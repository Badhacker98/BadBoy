...
from BadBoy.fns.helper import download_file
from BadBoy.fns.tools import get_google_images

from . import LOGS, get_help, get_string, udB, BadBoy_bot, BadBoy_cmd

__doc__ = get_help("help_autopic")


@BadBoy_cmd(pattern="autopic( (.*)|$)")
async def autopic(e):
...
if search := udB.get_key("AUTOPIC"):
    ...
    async def autopic_func():
        ...
        file = await BadBoy_bot.upload_file(filee)
        await BadBoy_bot(UploadProfilePhotoRequest(file))
        os.remove(filee)
