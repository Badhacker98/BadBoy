...
async def do_zip(event):
    ...
    if event.pattern_match.group(1).strip():
        await bash(
            f"zip -r --password {event.pattern_match.group(1).strip()} BadBoy.zip zip/*"
        )
    else:
        await bash("zip -r BadBoy.zip zip/*")
    k = time.time()
    xxx = await uploader("BadBoy.zip", "BadBoy.zip", k, xx, get_string("com_6"))
    await event.client.send_file(
        event.chat_id,
        xxx,
        force_document=True,
