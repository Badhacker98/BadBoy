...
from BadBoy.dB.notes_db import add_note, get_notes, list_note, rem_note
...
@BadBoy_cmd(pattern="addnote( (.*)|$)", admins_only=True)
async def an(e):
    ...
    BadBoy_bot.add_handler(notes, events.NewMessage())


@BadBoy_cmd(pattern="remnote( (.*)|$)", admins_only=True)
async def rn(e):
    wrd = (e.pattern_match.group(1).strip()).lower()
    chat = e.chat_id
    if not wrd:
...
@BadBoy_cmd(pattern="listnote$", admins_only=True)
async def lsnote(e):
