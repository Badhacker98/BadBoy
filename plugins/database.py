...
from . import Redis, eor, get_string, udB, BadBoy_cmd


@BadBoy_cmd(pattern="setdb( (.*)|$)", fullsudo=True)
async def _(ult):
...
@BadBoy_cmd(pattern="deldb( (.*)|$)", fullsudo=True)
async def _(ult):
...
@BadBoy_cmd(pattern="rendb( (.*)|$)", fullsudo=True)
async def _(ult):
    match = ult.pattern_match.group(1).strip()
    if not match:
