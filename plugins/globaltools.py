...
from BadBoy.dB.gban_mute_db import (
    gban,
    gmute,
    is_gbanned,
    is_gmuted,
...
@BadBoy_cmd(pattern="gpromote( (.*)|$)", fullsudo=True)
async def _(e):
...
@BadBoy_cmd(pattern="gdemote( (.*)|$)", fullsudo=True)
async def _(e):
    x = e.pattern_match.group(1).strip()
    BadBoy_bot = e.client
    if not x:
    ...
    if user:
        ...
        if user.isdigit():
            user = int(user)
        try:
            name = await BadBoy_bot.get_entity(user)
        except BaseException:
            return await e.eor(f"`No User Found Regarding {user}`", time=5)
        ...
        rank = "Not AdMin"
        c = 0
        async for x in BadBoy_bot.iter_dialogs():
            if (
                "group" in key.lower()
                and x.is_group
                or "group" not in key.lower()
...
@BadBoy_cmd(pattern="ungban( (.*)|$)", fullsudo=True)
async def _(e):
...
@BadBoy_cmd(pattern="gban( (.*)|$)", fullsudo=True)
async def _(e):
...
@BadBoy_cmd(pattern="g(admin|)cast( (.*)|$)", fullsudo=True)
...
@BadBoy_cmd(pattern="gucast( (.*)|$)", fullsudo=True)
...
@BadBoy_cmd(pattern="gkick( (.*)|$)", fullsudo=True)
async def gkick(e):
...
@BadBoy_cmd(pattern="gmute( (.*)|$)", fullsudo=True)
async def _(e):
...
@BadBoy_cmd(pattern="ungmute( (.*)|$)", fullsudo=True)
async def _(e):
...
async def list_gengbanned(event):
    ...
    if len(gbanned_users) > 4096:
        ...
        await x.reply(
            file="gbanned.txt",
            message=f"List of users GBanned by {inline_mention(BadBoy_bot.me)}",
        )
...
@BadBoy_cmd(pattern="gblacklist$")
async def blacklist_(event):
    await gblacker(event, "add")


@BadBoy_cmd(pattern="ungblacklist$")
