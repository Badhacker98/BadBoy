...
from BadBoy.fns.tools import metadata, translate
...
from . import inline_mention, is_url_ok, json_parser, mediainfo, BadBoy_cmd


@BadBoy_cmd(pattern="tr( (.*)|$)", manager=True)
async def _(event):
...
@BadBoy_cmd(pattern="bots( (.*)|$)", groups_only=True, manager=True)
async def _(ult):
...
async def magic(event):
    ...
    data = await async_searcher(
        "https://tiny.BadBoy.tech/api/new",
        data=data,
        post=True,
        re_json=True,
    )
