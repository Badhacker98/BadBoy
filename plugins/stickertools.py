...
async def pack_kangish(_):
    ...
    _packname = cmdtext or f"BadBoy Kang Pack By {_.sender_id}"
    typee = None
    if not local:
...
async def hehe(args):
    ...
    if message.photo:
        photo = io.BytesIO()
        photo = await BadBoy_bot.download_media(message.photo, photo)
    elif message.file and "image" in message.file.mime_type.split("/"):
        photo = io.BytesIO()
        await BadBoy_bot.download_file(message.media.document, photo)
        if (
        ...
        await BadBoy_bot.download_file(
    ...
    if photo:
        ...
        if not emoji:
            emoji = "🏵"
        if len(splat) == 3:
            pack = splat[2]  # User sent BadBoy_both
            emoji = splat[1]
        elif len(splat) == 2:
        ...
        if (
            ...
            async with BadBoy_bot.conversation("@Stickers") as conv:
                try:
                    await conv.send_message("/addsticker")
                except YouBlockedUserError:
                    LOGS.info("Unblocking @Stickers for kang...")
                    await BadBoy_bot(functions.contacts.UnblockRequest("stickers"))
                    await conv.send_message("/addsticker")
                ...
                await conv.get_response()
                await BadBoy_bot.send_read_acknowledge(conv.chat_id)
        else:
...
async def ultdestroy(event):
    ...
    await event.client.download_media(ult, "BadBoy.tgs")
    xx = await event.eor(get_string("com_1"))
    await bash("lottie_convert.py BadBoy.tgs json.json")
    ...
    file = await con.animated_sticker("json.json", "BadBoy.tgs")
    if file:
        await event.client.send_file(
            event.chat_id,
            file="BadBoy.tgs",
...
async def ultiny(event):
    ...
    im1 = Image.open("resources/extras/BadBoy_blank.png")
    if ik.endswith(".tgs"):
