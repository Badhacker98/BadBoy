...
from . import LOGS, asst, eor, events, get_string, udB, BadBoy_bot, BadBoy_cmd
...
async def source(e):
    ...
    if not SourceM.contains(y):
        ...
        BadBoy_bot.add_handler(autopost_func, events.NewMessage())
    else:
        await e.eor(get_string("cha_3"))


@BadBoy_cmd(pattern="dsource( (.*)|$)")
async def dd(event):
...
async def list_all(event):
    ...
    for channel in channels:
        name = ""
        try:
            name = get_display_name(await BadBoy_bot.get_entity(int(channel)))
        except BaseException:
            name = ""
        msg += f"\n=> **{name}** [`{channel}`]"
    msg += f"\nTotal {num} channels."
