...
from BadBoy.fns.info import get_uinfo

from . import HNDLR, LOGS, eod, eor, get_string, inline_mention, types, BadBoy_cmd
...
@BadBoy_cmd(pattern="pin$", manager=True, require="pin_messages", fullsudo=True)
async def pin(msg):
    if not msg.is_reply:
...
@BadBoy_cmd(pattern="purge( (.*)|$)", manager=True, require="delete_messages")
...
@BadBoy_cmd(pattern="pinned", manager=True, groups_only=True)
