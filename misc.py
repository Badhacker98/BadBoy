...
async def randomchannel(
    tochat, channel, range1, range2, caption=None, client=BadBoy_bot
):
...
async def allcmds(event, telegraph):
    ...
    for z in LIST.keys():
        ...
        for zz in LIST[z]:
            txt += HNDLR + zz + "\n"
        txt += "\n\n"
    t = telegraph.create_page(title="BadBoy All Cmds", content=[txt])
    await eor(event, f"All BadBoy Cmds : [Click Here]({t['url']})", link_preview=False)
