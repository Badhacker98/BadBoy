...
try:
    from BadBoy.fns.gDrive import GDriveManager
except ImportError:
    GDriveManager = None
...
from BadBoy.fns.helper import fast_download, progress
from BadBoy.fns.tools import Carbon, async_searcher, get_paste, telegraph_client
from BadBoy.startup.loader import Loader
...
_buttons = {
    "otvars": {
        "text": "Other Variables to set for @TeamBadBoy:",
        "buttons": [
            [
    ...
    "oofdm": {
        "text": "About [Dual Mode](https://t.me/BadBoyUpdates/18)",
        "buttons": [
            [
...
async def update(eve):
    ...
    if heroku_api:
        ...
        execl(sys.executable, sys.executable, "-m", "BadBoy")
...
async def changes(okk):
    ...
    if not match:
        try:
            ...
            return await okk.edit(
                f"**• BadBoy Userbot •**{cli}", file=img, buttons=button
            )
        except Exception as er:
            LOGS.exception(er)
    ...
    if len(changelog_str) > 1024:
        ...
        with open("BadBoy_updates.txt", "w+") as file:
            file.write(tl_chnglog)
        await okk.edit(
            get_string("upd_5"),
            file="BadBoy_updates.txt",
            buttons=button,
        )
        remove("BadBoy_updates.txt")
        return
    await okk.edit(
        changelog_str,
        buttons=button,
...
async def _(e):
    ...
    async with asst.conversation(e.sender_id) as conv:
        ...
        if GDrive._create_token_file(code=code.text):
            await conv.send_message(
                "`Success!\nYou are all set to use Google Drive with BadBoy Userbot.`",
...
async def _(e):
    if not e.is_private:
        return
    await e.edit(
        "Click Authorise and send the code.\n\nYou can use your own CLIENT ID and SECRET by [this](https://t.me/BadBoyUpdates/37)",
        buttons=[
            [
